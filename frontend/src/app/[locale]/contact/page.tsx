'use client';

import { useState } from 'react';
import { useParams } from 'next/navigation';
import type { Locale } from '@/lib/i18n';

const contactLabels: Record<string, Record<string, string>> = {
  'zh-hans': {
    heading: '联系我们',
    subtitle: '无论您有任何旅行想法或疑问，都欢迎联系我们。我们会在24小时内回复您。',
    formTitle: '发送咨询',
    name: '您的姓名',
    email: '电子邮箱',
    phone: '联系电话',
    destination: '感兴趣的目的地',
    travelers: '出行人数',
    message: '您的旅行想法或需求',
    submit: '提交咨询',
    success: '感谢您的咨询！我们会尽快与您联系。',
    contactInfo: '联系方式',
    orContact: '或直接联系我们',
    address: '地址',
    addressValue: '新加坡乌节路 #08-01 旅游中心大厦',
    hours: '服务时间',
    hoursValue: '周一至周六 9:00 - 18:00',
  },
  'zh-hant': {
    heading: '聯繫我們',
    subtitle: '無論您有任何旅行想法或疑問，都歡迎聯繫我們。',
    formTitle: '發送諮詢',
    name: '您的姓名',
    email: '電子郵箱',
    phone: '聯繫電話',
    destination: '感興趣的目的地',
    travelers: '出行人數',
    message: '您的旅行想法或需求',
    submit: '提交諮詢',
    success: '感謝您的諮詢！我們會盡快與您聯繫。',
    contactInfo: '聯繫方式',
    orContact: '或直接聯繫我們',
    address: '地址',
    addressValue: '新加坡烏節路 #08-01 旅遊中心大廈',
    hours: '服務時間',
    hoursValue: '週一至週六 9:00 - 18:00',
  },
  en: {
    heading: 'Contact Us',
    subtitle: 'Whether you have travel ideas or questions, feel free to reach out. We\'ll respond within 24 hours.',
    formTitle: 'Send an Inquiry',
    name: 'Your Name',
    email: 'Email Address',
    phone: 'Phone Number',
    destination: 'Interested Destination',
    travelers: 'Number of Travelers',
    message: 'Your travel ideas or requirements',
    submit: 'Submit Inquiry',
    success: 'Thank you for your inquiry! We\'ll get back to you soon.',
    contactInfo: 'Contact Information',
    orContact: 'Or contact us directly',
    address: 'Address',
    addressValue: '#08-01 Tourism Centre, Orchard Road, Singapore',
    hours: 'Business Hours',
    hoursValue: 'Mon - Sat 9:00 AM - 6:00 PM',
  },
};

export default function ContactPage() {
  const params = useParams();
  const locale = (params.locale as Locale) || 'zh-hans';
  const l = contactLabels[locale] || contactLabels['zh-hans'];
  const [submitted, setSubmitted] = useState(false);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSubmitted(true);
  };

  return (
    <>
      {/* Hero */}
      <section className="relative py-20 lg:py-28 bg-gradient-to-br from-primary-800 to-primary-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4">{l.heading}</h1>
          <p className="text-lg text-white/80 max-w-2xl mx-auto">{l.subtitle}</p>
        </div>
      </section>

      {/* Content */}
      <section className="py-16 lg:py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-16">
            {/* Form */}
            <div className="lg:col-span-3">
              <h2 className="text-2xl font-bold text-primary-900 mb-6">{l.formTitle}</h2>

              {submitted ? (
                <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
                  <svg className="w-16 h-16 text-green-500 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                  </svg>
                  <p className="text-lg text-green-800 font-medium">{l.success}</p>
                </div>
              ) : (
                <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{l.name} *</label>
                      <input type="text" required className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{l.email} *</label>
                      <input type="email" required className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{l.phone}</label>
                      <input type="tel" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">{l.travelers}</label>
                      <input type="number" min="1" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{l.destination}</label>
                    <input type="text" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">{l.message} *</label>
                    <textarea required rows={5} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 outline-none transition-colors resize-none" />
                  </div>
                  <button type="submit" className="w-full sm:w-auto px-8 py-3 bg-primary-500 text-white rounded-xl font-medium hover:bg-primary-600 transition-colors">
                    {l.submit}
                  </button>
                </form>
              )}
            </div>

            {/* Contact Info Sidebar */}
            <div className="lg:col-span-2">
              <div className="bg-warm-50 rounded-2xl p-8 space-y-8">
                <h3 className="text-xl font-bold text-primary-900">{l.orContact}</h3>

                <div className="space-y-6">
                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-medium text-primary-900">WhatsApp</p>
                      <a href="https://wa.me/6588888888" target="_blank" rel="noopener noreferrer" className="text-green-600 hover:text-green-700">+65 8888 8888</a>
                    </div>
                  </div>

                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-medium text-primary-900">{locale === 'en' ? 'Phone' : '电话'}</p>
                      <a href="tel:+6588888888" className="text-primary-600 hover:text-primary-700">+65 8888 8888</a>
                    </div>
                  </div>

                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-medium text-primary-900">{locale === 'en' ? 'Email' : '邮箱'}</p>
                      <a href="mailto:hello@starvoyage.sg" className="text-primary-600 hover:text-primary-700">hello@starvoyage.sg</a>
                    </div>
                  </div>

                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-medium text-primary-900">{l.address}</p>
                      <p className="text-gray-600">{l.addressValue}</p>
                    </div>
                  </div>

                  <div className="flex items-start gap-4">
                    <div className="w-10 h-10 bg-primary-100 rounded-lg flex items-center justify-center shrink-0">
                      <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                    </div>
                    <div>
                      <p className="font-medium text-primary-900">{l.hours}</p>
                      <p className="text-gray-600">{l.hoursValue}</p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </section>
    </>
  );
}
